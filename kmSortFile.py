...
for i in range(num_clusters):
    ...
    # for each gene, flag the current cluster as test, and others as compared
    # calculate the mean for each and do a two-sample t-test to see if the cluster
    # gene expression is uniquely expressed
...
########## this is a check to only store one differently expr gene ###########
            ...
            # else, we annotate this diff expressed gene to the corresponding
            # cluster
            else:
                diff_expression[j] = (i, p_value)
...
# output to a file named after the original name, with 'sorted' tagged behind
output = open(name + '-sorted.gct', 'w')

# write the header and # of rows/columns, also write the names of sample
output.write(header + '\n' + info + '\n')
output.write("NAME" + '\t' + "Description" + '\t' + '\t'.join(sample_names) + '\n')


# construct a int array containing length of num of rows, start writing in rows
# that are differently expressed by each cluster one by one, simultaneously
# remove them from this array. Finally, write the rest rows IN ORDER to the file
rest_rows = [x for x in range(num_rows)]

# iterate through the whole dictionary for each cluster, so we can print out
# the corresponding differently expressed genes in cluster order
...
for i in range(len(rest_rows)):
    output.write(probe_names[rest_rows[i]] + '\t' + description[rest_rows[i]]
                + '\t')
